...
<header class="header">
    ...
    <form class="header__search search" role="search" method="get" action="<?= get_home_url(); ?>">
        <div class="search__container">
            <label for="s" class="search__label"><?= __('Votre recherche', 'Aline-db-antilope'); ?></label>
            <input type="text" name="s" id="s" placeholder="Votre recherche" class="search__input" value="<?= get_search_query(); ?>" />
            <button type="submit" class="search__btn"><?= __('Rechercher', 'Aline-db-antilope'); ?></button>
        </div>
    </form>
</header>